...
##		}
	 ...
	 *
	 * @post	Valid row ID needs to be passed in order to get a valid row from the DB.
	 * @param	integer	$id	Row ID
	 * @return	mixed	Collection of rows, but should be either empty or a valid single row.
	 */
	...
	/**
	 * Remove a particular row from the notifications tble given the row ID.
	 *
	 * @post	Database row deleted if ID is valid
	 * @param	integer	$id	Row ID
	 * @return 	integer	Count of rows removed
	 */
	...
	/**
	 * Remove all entires for a particular emaill address from the notifications table.
	 *
	 * @post	Database row(s) deleted if email is valid
	 * @param	string	$email_address 	Customers email address.
	 * @return 	integer	Count of rows removed
	 */
	public function DeleteByEmail($email_address)
	{
		return \DB::table('notifications')->where(['email_address'=>$email_address])->delete();
	}
}
